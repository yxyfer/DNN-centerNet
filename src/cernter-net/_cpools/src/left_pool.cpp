...
std::vector<torch::Tensor> pool_forward(
    ...
    torch::Tensor input_temp  = input.slice(3, width - 1);
    torch::Tensor output_temp = output.slice(3, width - 1);
    output_temp.copy_(input_temp);

    torch::Tensor max_temp;
    for (int64_t ind = 1; ind < width; ++ind) {
        input_temp  = input.slice(3, width - ind - 1);
        output_temp = output.slice(3, width - ind);
        max_temp    = output.slice(3, width - ind - 1);
...
std::vector<torch::Tensor> pool_backward(
    ...
    auto input_temp = input.slice(3, width - 1);
    max_val.copy_(input_temp);

    max_ind.fill_(width - 1);

    auto output_temp = output.slice(3, width - 1);
    auto grad_output_temp = grad_output.slice(3, width - 1);
    ...
    for (int32_t ind = 1; ind < width; ++ind) {
        ...
        grad_output_temp = grad_output.slice(3, width - ind - 1);
