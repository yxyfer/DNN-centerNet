...
std::vector<torch::Tensor> pool_forward(
    ...
    torch::Tensor input_temp  = input.slice(2, 0);
    torch::Tensor output_temp = output.slice(2, 0);
    output_temp.copy_(input_temp);

    torch::Tensor max_temp;
    for (int64_t ind = 0; ind < height - 1; ++ind) {
        input_temp  = input.slice(2, ind + 1);
        output_temp = output.slice(2, ind);
        max_temp    = output.slice(2, ind + 1);
...
std::vector<torch::Tensor> pool_backward(
    ...
    auto input_temp = input.slice(2, 0);
    max_val.copy_(input_temp);

    max_ind.fill_(0);

    auto output_temp = output.slice(2, 0);
    auto grad_output_temp = grad_output.slice(2, 0);
    ...
    for (int32_t ind = 0; ind < height - 1; ++ind) {
        ...
        grad_output_temp = grad_output.slice(2, ind + 1);
